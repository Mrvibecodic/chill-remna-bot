...
func (c *Client) CreatePaymentSaving(ctx context.Context, value, currency, description, returnURL string, telegramID int64, months int, plan string, save bool) (*Payment, error) {
	...
	// Сам бот тариф из метаданных пока не читает (условия сделки едут в строке
	// счёта): это опознание платежа на стороне платёжки — по нему видно, что
	// именно продано, когда строки счёта в базе уже нет.
	if plan != "" {
		meta["plan"] = plan
	}
	if save {
