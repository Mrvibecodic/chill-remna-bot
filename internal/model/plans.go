...
//
// Для Stars это ещё и единственный носитель тарифа: их payload менять нельзя
// (см. комментарий к таблице purchase_intents).
type PurchaseIntent struct {
	TelegramID int64
	PlanCode   string
	Months     int
	Days       int
	// Snapshot — условия, зафиксированные при выставлении счёта. Заполняется
	// там, где счёта в базе нет и снимок больше положить некуда (Stars).
