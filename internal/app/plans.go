...
func (a *App) buyMonthsOrAsk(ctx context.Context, chatID int64) int {
	...
	// Проверяем не только «выбор есть», но и «этот срок ещё продаётся»:
	// админ мог убрать базовую цену, пока экран способов висел в переписке, а
	// у P2P и ЮKassa рядом остаётся переопределение цены — и срок, снятый с
	// витрины, продолжал бы продаваться ими сутки.
	if in != nil && a.periodOnSale(in.Months) {
		return in.Months
	}
