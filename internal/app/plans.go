...
//
// Известное ограничение ключа «человек + способ + срок»: два открытых счёта на
// РАЗНЫЕ тарифы с одним сроком делят одну строку — последний выставленный
// побеждает. Payload несёт только срок, различить счета при оплате нечем.
// Деньги защищены сверкой суммы (starsSnapshotForAmount): при разных ценах
// чужой счёт отклонит предпроверка; совпали и срок, и звёздная цена — оплата
// первого счёта применит условия второго, выставленного тем же человеком по
// той же цене. Осознанный компромисс, а не дыра.
