...
func TestMiniCheckoutInvalidMonths(t *testing.T) {
	a := &App{}
	r := a.MiniCheckout(context.Background(), 1, 7, "balance", false, false) // 7 not in PlanMonths
	if r.OK || r.Error == "" {
...
func TestMiniCheckoutUnknownMethod(t *testing.T) {
	a := &App{}
	r := a.MiniCheckout(context.Background(), 1, 1, "nope", false, false)
	if r.OK || r.Error == "" {
...
func TestMiniCheckoutUnconfiguredExternal(t *testing.T) {
	...
	r := a.MiniCheckout(context.Background(), 1, 1, "yookassa", false, false)
	if r.OK || r.Error == "" {
