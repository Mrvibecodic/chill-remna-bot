...
func (a *App) repairTarget(ctx context.Context, panel *remnawave.Client, tgID int64, snap *model.PlanSnapshot, full bool) bool {
	...
	// Проданный безлимит трафика сверка НЕ восстанавливает намеренно. Отличить
	// «потолок остался от предыдущего образа» от «потолок выставил админ
	// руками» по панели нельзя, а сверка ходит по всем подписчикам каждые 12
	// часов: она отбирала бы у админа ручное ограничение раз за разом. У тех,
	// кто купил безлимит до этой версии, он применится при следующей покупке
	// или продлении.
