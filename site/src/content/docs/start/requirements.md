...
5. **API-токен панели Remnawave** — создаётся в дашборде панели (раздел с API-ключами, роль API). Его **не** надо вписывать в файл — введёте в мастере внутри бота.
   На панели **3.x** у токенов есть скоупы. При обновлении панели они мигрируют сами, но если создаёте токен заново — возьмите готовый список из [«Права токена панели»](/reference/panel-scopes/).
   Минимум, без которого бот не стартует: `system:remnawave-health` (с неё начинается проверка связи), `users:` stream, by-username, by-id, list, create, update, delete, reset-traffic, revoke-subscription, а также `internal-squads:list`, `external-squads:list` и `system:stats`.
