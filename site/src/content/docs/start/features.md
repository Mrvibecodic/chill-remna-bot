...
- **Триал** (бесплатный пробный период) — включается одной кнопкой; один раз на аккаунт. Пока триал идёт, витрина закрыта — купить платный тариф можно в день окончания триала, чтобы бесплатные дни не сгорели; отдельным тумблером покупку во время триала можно разрешить.
...
- **Свои устройства** — список подключений (в боте по кнопке «Мои устройства» под счётчиком, в Mini App и веб-кабинете — карточкой): платформа и ОС, модель, приложение клиента, отпечаток и дата подключения. Какие из этих полей видит человек, решает владелец бота (**Интерфейс → 📱 Устройства**).
