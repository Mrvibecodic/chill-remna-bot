...
server {
    ...
    location / {
        ...
        proxy_set_header X-Forwarded-Proto $scheme;  # обязательно: иначе бот ответит «требуется HTTPS»

        # Именно $remote_addr, а НЕ $proxy_add_x_forwarded_for: второй
        # дописывает реальный адрес в конец строки, присланной клиентом, —
        # и тогда клиент сам задаёт себе ключ ограничителя частоты.
...
:::caution[Не используйте `$proxy_add_x_forwarded_for`]
Эта переменная склеивает то, что прислал клиент, с реальным адресом и ставит
реальный **последним**. Ограничитель частоты после этого ключуется строкой,
которую задаёт сам клиент, — и защита от перебора паролей кабинета перестаёт
работать. То же с `CF-Connecting-IP` и `X-Real-IP`: если прокси их не
перезаписывает, клиент присылает их сам.
:::

:::note[HTTPS требуется на всех ручках]
Требование распространяется на весь API, а не только на вход в кабинет: по этим
же путям ходят ключ доступа, ссылка на подписку и чек об оплате. Исключение —
`/webhook/` и `/healthz`: они ходят внутрь сети. Если ваш прокси не выставляет
`X-Forwarded-Proto`, бот ответит `426`; поправьте конфиг либо, как временную
меру, запустите с `ALLOW_PLAIN_HTTP=1`.

Заголовку верят **только от локального или частного адреса** — то есть от
прокси на той же машине или в той же docker-сети. Если ваш прокси стоит на
ДРУГОМ сервере с публичным адресом, назовите его в `TRUSTED_PROXIES`
(`TRUSTED_PROXIES=203.0.113.9`), иначе кабинет уйдёт в бесконечный редирект на
HTTPS, API будет отвечать `426`, а все посетители попадут в одну корзину
ограничителя частоты и начнут получать `429`.
:::
...
Кабинет и мини-апп отдают `noindex`/robots, `Referrer-Policy: no-referrer`, `nosniff`, HSTS, anti-fingerprint, а флаги хостятся локально — но это работает в полную силу только если **хост не светится в CT-логах и у сканеров**, поэтому wildcard-серт + скрытый origin для прод-кабинета критичны.
